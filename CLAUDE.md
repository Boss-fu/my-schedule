...
## Supabase 資料表（線上實際）
`profiles`(role: teacher/parent)、`students`、`parent_students`(多對多)、`lessons`、`messages`(有 `author_role`、`parent_id`)、`student_files`(有 `uploader_id`、`parent_id`)、`issued_invoices`(student_id, month, PK)、`site_settings`(key/value，如 finance_profile)。Storage bucket：`exam-papers`。
...
## 待辦（未完成）
- **F**：把系統擴成「所有接課」——補習班兼職課次（無學生、無學費單）+ 可蓋章的**薪資對帳單**。大型新模組，需先設計 + 新資料表。
- **推播通知**：iPhone 系統通知需 VAPID + `push_subscriptions` 表 + Supabase Edge Function 發送（使用者須部署）。使用者說之後再做。
